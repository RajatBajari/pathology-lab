    ...
    try {
        ...
        if (newUser) {
            ...
            res.json({
                ...
                "error": "Some Technical Issue", "message": "fail in Patient posting"
            })
        }
    } catch (err) {
        res.status(404).json({
...
exports.updatePatient = async (req, res) => {
    try {
        const body = req.body;
        const { id } = req.params;
        const updatedPatient = await Patient.updateOne({ _id: id }, body); if (updatedPatient) {
            res.status(200).json({
                message: "updated Successfully", data: updatedPatient
            })
        }
    } catch (err) {
        res.status(404).json({
...
exports.deletePatient = async (req, res) => {
    try {
        ...
        if (deletedUser.deletedCount > 0) {
            res.status(200).json({
                message: "patient deleted successfully", deletedUser: deletedUser
            })
        } else {
            res.status.json({
